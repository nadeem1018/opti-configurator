            ...
            if (data.DataForSelectedFeatureModelItem.length > 0) {
                  ...
                  for (let imodelheader = 0; imodelheader < this.ModelHeaderData.length; imodelheader++) {
                    if (this.ModelHeaderData[imodelheader].parentmodelid != undefined) {
                      if (this.ModelHeaderData[imodelheader].parentmodelid == parentmodelid) {
                        ...
                        let uniqueKey = this.ModelHeaderData[imodelheader].unique_key
                        this.ModelHeaderData.splice(imodelheader, 1);
                        imodelheader = imodelheader - 1;
                        this.removemodelheaderdatatable(removemodelheaderid,uniqueKey)
              ...
              if (type == 1) {
                  ...
                  for (var i = 0; i < data.DataForSelectedFeatureModelItem.length; ++i) {
                    ...
                    if (data.DataForSelectedFeatureModelItem.length > 0) {
                      ...
                      if (isExist.length == 0) {
                        ...
                        let nodeId = '';
                        if(data.DataForSelectedFeatureModelItem[i].OPTM_TYPE == 1) {
                            uniqueKey = feature_model_data.unique_key;
                            nodeId = feature_model_data.unique_key;
                        } else if( data.DataForSelectedFeatureModelItem[i].OPTM_TYPE == 2) {
                            uniqueKey = data.DataForSelectedFeatureModelItem[i].unique_key;
                      ...
                      if (checkeddefault == true) {
                        ...
                        if (parentarray.length > 0) {
                          ...
                          if (data.DataForSelectedFeatureModelItem[i].OPTM_TYPE == 2) {
                            ...
                            this.setItemDataForFeature(itemData, parentarray, propagateqtychecked, propagateqty, parentarray[0].feature_code, parentarray[0].HEADER_LINENO,type,parentArrayElemType);
                          }
                ...
                // let parentarray = this.ModelHeaderData.filter(function (obj) {
                  ...
                  this.setItemDataForFeature(data.DataForSelectedFeatureModelItem, parentarray, propagateqtychecked, propagateqty, parentarray[0].feature_code, parentarray[0].OPTM_LINENO,type,parentArrayElemType);
                }
                else if (type == 2) {
                  if (parentarray[0].OPTM_PROPOGATEQTY == "Y") {
...
setDtFeatureDataWithDefault(dtFeatureDataWithDefault, DataForSelectedFeatureModelItem, feature_model_data, parentmodelid, parentarray, propagateqtychecked, data,type) {
  for (var idtfeature in dtFeatureDataWithDefault) {
    ...
    setItemDataForFeature(ItemData, parentarray, propagateqtychecked, propagateqty, tempfeaturecode, lineno,type,parentArrayElemType) {
      ...
      if (ItemData.length > 0) {
        if (parentArrayElemType == "radio") {
         /* for (let i = 0; i < this.feature_itm_list_table.length; i++) {
            if (parentarray[0].OPTM_TYPE == 1 && parentArrayElemType == "radio" && type == 1) {
              if(this.feature_itm_list_table[i].parent_featureid != null && this.feature_itm_list_table[i].parent_featureid != "" && this.feature_itm_list_table[i].parent_featureid != undefined) {
                if (this.feature_itm_list_table[i].parent_featureid == parentarray[0].OPTM_FEATUREID) {
                  this.feature_itm_list_table.splice(i, 1);
                  i = i - 1;
                }
              }else {
    ...
    for (var imodelchecked in this.ModelBOMDataForSecondLevel) {
      if (feature_model_data.OPTM_TYPE == 2) {
        if (this.ModelBOMDataForSecondLevel[imodelchecked].OPTM_FEATUREID == feature_model_data.OPTM_FEATUREID && this.ModelBOMDataForSecondLevel[imodelchecked].OPTM_ITEMKEY == feature_model_data.OPTM_ITEMKEY && this.ModelBOMDataForSecondLevel[imodelchecked].unique_key == feature_model_data.OPTM_unique_key) {
    ...
    setModelDataInOutputBom(getmodelsavedata, SelectedAccessory, modelHeaderdata, selectedAccessoryHeader) {
     ...
     removemodelheaderdatatable(removemodelheaderid,uniqueKey) {
       for (let iremove = 0; iremove < this.ModelHeaderData.length; iremove++) {
         if (this.ModelHeaderData[iremove].parentfeatureid == removemodelheaderid && this.ModelHeaderData[iremove].unique_key == uniqueKey) {
           let removemodelheaderidchild = this.ModelHeaderData[iremove].OPTM_FEATUREID
           let modelHeaderUniqueKey = this.ModelHeaderData[iremove].unique_key
           ...
           this.removemodelheaderdatatable(removemodelheaderidchild,modelHeaderUniqueKey)
         }
       }
     }
